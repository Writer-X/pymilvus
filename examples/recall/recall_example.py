...
for t in range(block_num):
    ...
    #         [vec.astype(np.float32).tolist() for vec in data[counter: counter + block_size]]
    #         ]
    entities = []
    i_list = [i for i in range(counter, counter + block_size)]
    vec_list = [vec.astype(np.float32).tolist() for vec in data[counter: counter + block_size]]
    entities.append(i_list)
    entities.append(vec_list)
...
# search
print(f"\nSearch...")
res = collection.search(test[0:10], "vec", {"metric_type": "L2"}, limit=100)
print(res)
